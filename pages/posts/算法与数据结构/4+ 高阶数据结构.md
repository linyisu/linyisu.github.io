...
这样转换后：
- **修改边 `(u, v)` 的权值**：等价于修改深度更深的那个点（假设是 `v`）的点权，操作为对单点 `v` 修改：`seg.modify(dfn[v], dfn[v], ...)`
- **查询路径 `(u, v)` 的边权和**：等价于查询路径上所有点的点权和，但需要**排除LCA的点权**，因为 `lca(u, v)` 的点权代表的是 `(fa[lca], lca)` 这条边的权值，而这条边不在这条路径上，所以操作为 `queryPath(u, v) - query_single_point(lca(u, v))`
